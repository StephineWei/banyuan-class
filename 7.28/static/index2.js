...
window.onload = function () {
    ...
    var smallImg = $('.small-img')[0];
    // var objBigBox = document.getElementsByClassName("big-box")[0];
    var objBigBox = $('.big-box')[0];

    // var objBigBoxImage = objBigBox.getElementsByTagName("img")[0];
    var  objBigBoxImage = $('.big-box').find('img')[0];
    console.log(objBigBoxImage)


    //鼠标移入事件
    smallImg.onmouseover = function () {
        $('.float-box')[0].style.display = "block"
        $('.big-box')[0].style.display = "block"
    }
    //鼠标移出事件
    smallImg.onmouseout = function () {

        $('.float-box')[0].style.display = "none"
        $('.big-box')[0].style.display = "none"
    }
    //鼠标移动事件
